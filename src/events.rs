//! Eventos assíncronos do banco (`POST_EVENT` / `isc_que_events`).
//!
//! Uma conexão registra interesse em eventos nomeados; quando outra conexão faz
//! `POST_EVENT 'nome'` e commita, o servidor notifica de forma assíncrona por um
...
impl Connection {
    ...
    ///
    /// ```text
    /// let mut ev = conn.listen_events(&["minha_tabela_mudou"]).await?;
    /// let disparados = ev.wait(&mut conn).await?;   // bloqueia até um POST
    /// ev.cancel(&mut conn).await?;
    /// ```
    pub async fn listen_events(&mut self, names: &[&str]) -> Result<EventListener> {
        if names.is_empty() {
        ...
        let sock = TcpStream::connect((ip, port)).await?;
        ...
        que_events(self, &names, &counts, event_id).await?;

        Ok(EventListener { aux, event_id, names, counts })
    }
}
...
impl EventListener {
    ...
    pub async fn wait(&mut self, conn: &mut Connection) -> Result<Vec<String>> {
        loop {
            // Lê um op_event do canal auxiliar (bloqueia até chegar).
            let code = read_op(&mut self.aux).await?;
            if code != op::EVENT {
            ...
            let _db = self.aux.read_i32().await?;
            let epb = self.aux.read_bytes().await?;
            let _ast = self.aux.read_i32().await?;
            let _rid = self.aux.read_i32().await?;
            ...
            que_events(conn, &self.names, &self.counts, self.event_id).await?;
...
fn parse_aux_addr(data: &[u8], fallback: Option<IpAddr>) -> Result<(IpAddr, u16)> {
    if data.len() < 8 {
        return Err(Error::protocol("resposta de op_connect_request sem sockaddr"));
    }
...
mod tests {
    ...
    fn parse_aux_addr_from_sockaddr() {
        ...
        let data = [0x02, 0x00, 0x8e, 0x81, 0x7f, 0x00, 0x00, 0x01, 0, 0, 0, 0, 0, 0, 0, 0];
